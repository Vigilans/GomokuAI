...
//    auto get_agent = [&](Player p) { return agents[p == Player::White]; };
//
//    if (shuffle) {
//        srand(time(nullptr));
//        if (rand() % 2) {
...
//            cout << "Black's proposals: ";
//            for (auto move : proposals) {
//                cout << to_string(move) << ", ";
//            }
//            auto [_, rival] = get_agent(Player::White);
//            auto response = rival.respondProposals(std::move(proposals));
...
inline int ConsoleInterface(Agent& agent0, Agent& agent1) {
    using namespace std;
    /////////////////////
	string first_player = get_name("先手"),
		second_player = get_name("后手");
	char start_time[300];
	string header_info;
	string game = "C5";
    ...
    time_t timep;
    time (&timep);
    strftime(start_time, sizeof(start_time), "%Y.%m.%d %H:%M 安徽",localtime(&timep) );
    ...
    for (auto cur_player = Player::Black; ;) {
        ...
        if (move == Position{ -1, -1 }) {
			...
			game_records << (cur_player == Player::Black ? ";B(" : ";W(") << move.x() << "," << move.y() << ")";
